...
try:
    from models.shipyolo.hx_function import Conv2dBatchLeaky, BasicRFA, Add_UpFuseStage, CBAM, DownFuseStage
except:
    from hx_function import Conv2dBatchLeaky, BasicRFA, Add_UpFuseStage, CBAM, DownFuseStage
    
class HX_AtFPN(nn.Module):
    def __init__(self):
        ...
        self.layer0 = nn.Sequential(
            ...
            Conv2dBatchLeaky(in_channels=1024, out_channels=512, kernel_size=1, stride=1, pad=1),
            # DSpp
            BasicRFA(in_planes=512, out_planes=512),
            # CBL(3)
        ...
        self.layer1_b = nn.Sequential(
            ...
            Conv2dBatchLeaky(in_channels=256, out_channels=512, kernel_size=3, stride=1, pad=1),
            Conv2dBatchLeaky(in_channels=512, out_channels=256, kernel_size=1, stride=1, pad=1),
            Conv2dBatchLeaky(in_channels=256, out_channels=512, kernel_size=3, stride=1, pad=1),
            Conv2dBatchLeaky(in_channels=512, out_channels=256, kernel_size=1, stride=1, pad=1),
        )
