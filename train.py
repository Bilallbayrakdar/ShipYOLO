...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    # Model
    model = ShipYOLO(num_classes=14, deploy=True).to(device)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                loss, loss_items = compute_loss_hx(pred, targets.to(device), model)  # loss scaled by batch_size
                if rank != -1:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='data/wsodd.yaml', help='data.yaml path')
